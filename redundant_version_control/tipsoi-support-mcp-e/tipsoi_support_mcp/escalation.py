...
Files a ticket into the support team's ClickUp via the documented REST API
(create a task in the List the public form feeds). ClickUp exposes no documented
public-form submission API and a form's field IDs are workspace-specific UUIDs,
so the REST API is the stable path. A webhook mode and an always-return-the-
ticket fallback mean an escalation is never silently lost.

Env: SUPPORT_TICKET_MODE (auto|clickup_api|webhook|off), CLICKUP_API_TOKEN,
CLICKUP_LIST_ID, CLICKUP_CUSTOM_FIELDS, SUPPORT_TICKET_WEBHOOK,
...
async def _submit_clickup_api(ticket: Ticket) -> dict[str, Any]:
    ...
    body: dict[str, Any] = {"name": ticket.task_name, "description": ticket.to_markdown()}
    ...
    if cf_map_raw:
        try:
            ...
            custom_fields = [
                ...
                for key, uuid in cf_map.items() if key in data and data[key]
            ]
            if custom_fields:
    ...
    if resp.status_code in (200, 201):
        data = resp.json()
        return {"filed": True, "channel": "clickup_api",
                "ticket_id": data.get("id"), "ticket_url": data.get("url")}
    return {"filed": False, "channel": "clickup_api",
...
async def _submit_webhook(ticket: Ticket) -> dict[str, Any]:
    ...
    return {"filed": False, "channel": "webhook",
...
async def submit(ticket: Ticket) -> dict[str, Any]:
    ...
    try:
        if mode == "clickup_api":
            ...
            result = {"filed": False, "channel": "none",
                      "note": ("No ticket backend configured (SUPPORT_TICKET_MODE=off). "
                               "File the formatted ticket below manually.")}
