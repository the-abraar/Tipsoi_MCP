...
**With Gemini, or for other users:** see `examples/GEMINI_SETUP.md` (use it from
the Gemini API) and `gateway/README.md` (give others revocable keys without
sharing your Gemini key).
...
the assistant won't follow the support playbook as tightly. (The Gemini bridge
and gateway load this automatically.)
...
> (`tipsoi-mcp`)**, not this one. The two can run side by side, and the host
> model will route to whichever fits the request.
