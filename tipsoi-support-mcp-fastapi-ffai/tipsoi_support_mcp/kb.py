...
# Keep ASCII word chars AND the Bengali Unicode block (U+0980–U+09FF) so the
# bilingual KB is searchable in both languages.
_TOKEN_RE = re.compile(r"[a-z0-9]+|[ঀ-৿]+")

# Very small English stop-word set — enough to stop common words from
# dominating BM25 scores without needing an NLP library.
_STOPWORDS = {
...
def _parse_front_matter(raw: str) -> tuple[dict[str, Any], str]:
    """Return (metadata, body). Tolerant mini-YAML — handles the simple
    scalar / quoted / ``[a, b]`` list shapes the KB actually uses."""
    m = _FM_RE.match(raw)
    if not m:
        return {}, raw
    meta: dict[str, Any] = {}
...
@dataclass
class Article:
    doc_id: str            # stable id, e.g. "03-leave-management/leave-management-overview"
    title: str
    category: str
    description: str
    tags: list[str]
    ...
    def summary(self, max_chars: int = 320) -> str:
        """First meaningful prose block, for search-result previews."""
        text = self.description.strip()
        if not text:
            for block in re.split(r"\n\s*\n", self.body):
                block = block.strip()
                # skip headings / horizontal rules / tables
...
class _BM25:
    """Minimal BM25 (Okapi) over the loaded articles."""

    K1 = 1.5
    B = 0.75
    # Weight title + tags by repeating their tokens so a query term that hits
    # the title outranks the same term buried in the body.
    TITLE_BOOST = 3
    TAG_BOOST = 2
    ...
    def __init__(self, articles: list[Article]):
        ...
        # Precompute term-frequency maps per doc
...
# ---------------------------------------------------------------------------
# Load + public API (cached singletons)
# ---------------------------------------------------------------------------
...
def get_article(doc_id: str) -> dict[str, Any] | None:
    ...
    if not art:
        # tolerate ids passed with a .md suffix or leading slash
